...
const ListRegisProve = () => {
  ...
  useEffect(()=>{
    ...
    const filtrar = (terminoBusqueda)=>{
      var resultadosbusqueda=tablaprovee.filter((elemento)=>{
          if(elemento.nombreProveedor.toString().toUpperCase().includes(terminoBusqueda.toUpperCase())){
              return elemento; 
          }
      }); 
    ...
    const petiPUT=async()=>{
      gestorSeleccionado.telefono=parseInt(gestorSeleccionado.telefono);
      await axios.put(baseUrl+"/"+gestorSeleccionado.idProveedor,gestorSeleccionado).then(resp=>{
          ...
          alert('Los datos del proveedor: '+ gestorSeleccionado.nombreProveedor+' fueron actualizados correctamente')
  ...
  const petiDELETE=async()=>{
    await axios.delete(baseUrl+"/"+gestorSeleccionado.idProveedor)
    .then(resp=>{
      ...
      alert('Los datos del proveedor: '+ gestorSeleccionado.nombreProveedor+' fueron eliminados correctamente')
  ...
  return (
    <div>
       ...
       <br/>
       <button className="btn btn-success " onClick={()=>abrircerrarModalInsertar()}>NUEVO PROVEEDOR</button>{"   "}
       ...
       <br/>
          ...
          <tbody className='text-start'>
            ...
            <tr key={ordenes.idProveedor}>
              ...
              <td>
                  ...
                  <button className='btn btn-primary'  onClick={()=>seleccionarGestor(ordenes,"Eliminar")}>ELIMINAR</button>
              </td>
            </tr>
            ))}
        ...
        <Modal isOpen={modalInsertar}>
                ...
                <ModalFooter>
                    <button className="btn btn-info" onClick={()=>petiPost()}>Registrar</button>{" |"}
            ...
            <Modal isOpen={modalEditar}>
                ...
                <ModalBody>
                    ...
                    <label>IdProveedor:</label>
                      <input type="text" className="form-control" name="idProveedor" value={gestorSeleccionado && gestorSeleccionado.idProveedor} readOnly/>
                      <label>Nombre:</label>
                      <input type="text" className="form-control" name="nombreProveedor" onChange={handlechange}  value={gestorSeleccionado && gestorSeleccionado.nombreProveedor} /><br/>
                      <label>Direccion:</label>
                      <input type="text" className="form-control" name="direccion" onChange={handlechange}  value={gestorSeleccionado && gestorSeleccionado.direccion}/><br/>
                      <br/>
                      <label>Telefono:</label>
                      ...
                      <br/>
                      <label>Estado :</label>
                      <input type="text" className='form-control' name="estado"  onChange={handlechange} value={gestorSeleccionado && gestorSeleccionado.estado}/>
                      <br/>
                    </div>
                </ModalBody>
                <ModalFooter>
                    <button className="btn btn-info" onClick={()=>petiPUT()}>Actualizar Dato</button>{" |"}
            ...
            <Modal isOpen={modalEliminar}>
              <ModalBody>
                ¿Estas seguro que deseas eliminar al proveedor: {gestorSeleccionado && gestorSeleccionado.nombreProveedor} ?
              </ModalBody>
              <ModalFooter>
